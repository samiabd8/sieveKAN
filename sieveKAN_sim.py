...
class ResidualSieveKAN(nn.Module):
    ...
    def analyze_representation_sparsity_enhanced(self, relative_fraction=KAN_RELATIVE_THRESHOLD_FRACTION):
...
def compute_dml_estimator_corrected(Y, D, m_hat, g_hat, theta_true=None):
    V_hat = D - m_hat
    U_hat = Y - g_hat
...
def cross_fitting_slfn_with_metrics(X, D, Y, n_folds=N_FOLDS, epochs=500, lr=0.001, batch_size=16):
    n = X.shape[0]
    indices = np.arange(n)
...
def cross_fitting_lasso_with_metrics(X, D, Y, n_folds=N_FOLDS, lambda_reg=LASSO_ALPHA):
    n = X.shape[0]
    indices = np.arange(n)
    np.random.shuffle(indices)
...
def cross_fitting_kan_with_metrics(X, D, Y, n_folds=N_FOLDS, n_epochs=KAN_N_EPOCHS, lr=KAN_LR,
    ...
    for fold_idx in range(n_folds):
        ...
        sparsity_analysis_m = model_m.analyze_representation_sparsity_enhanced()
        sparsity_analysis_g = model_g.analyze_representation_sparsity_enhanced()
...
def dml_highdim_simulation_with_complete_metrics(n=N_OBS, d=DIM, s=S_SPARSE, theta=THETA, n_folds=N_FOLDS,
    ...
    m_hat_kan, g_hat_kan, kan_metrics = cross_fitting_kan_with_metrics(
    ...
    dml_results_kan = compute_dml_estimator_corrected(Y, D, m_hat_kan, g_hat_kan, theta_true=theta)
    ...
    m_hat_lasso, g_hat_lasso, lasso_metrics = cross_fitting_lasso_with_metrics(
        X, D, Y, n_folds=n_folds, lambda_reg=lambda_reg
    )
    lasso_time = time.time() - lasso_start_time

    dml_results_lasso = compute_dml_estimator_corrected(Y, D, m_hat_lasso, g_hat_lasso, theta_true=theta)
    ...
    m_hat_slfn, g_hat_slfn, slfn_metrics = cross_fitting_slfn_with_metrics(
        X, D, Y, n_folds=n_folds, epochs=500, lr=0.001, batch_size=32
    )
    slfn_time = time.time() - slfn_start_time

    dml_results_slfn = compute_dml_estimator_corrected(Y, D, m_hat_slfn, g_hat_slfn, theta_true=theta)
...
if __name__ == '__main__':
    ...
    results = dml_highdim_simulation_with_complete_metrics(
